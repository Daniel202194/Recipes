...
router.get('/randomRecipes', async (req, res, next) => {
  try {
    ...
    while (i < results.length) {
      while (results[i].instructions === undefined || results[i].image  === undefined) {
...
router.get("/recipepage/recipeID/:recipeID", async (req, res, next) => {
  try {
    ...
    if (get_recipe.length == 0) {
      ...
      let relevant = await relevantData(get_recipe, req);
...
router.get("/search/query/:searchQuery/amount/:num", async (req, res) => {
  try {
      ...
      let emptyRecipes=[];
      if(recipes.length == 0){
          res.status(200).send(emptyRecipes)
      }else{
...
async function isInFavourite(id) {
  const isrecipeThere = await DButils.execQuery(`select id from favoritesRecipes where id='${id}' `);
...
async function addToWatchTable(recipID, req) {
  if (req.session.user_id) {
    ...
    const check_if_exist2 = await DButils.execQuery(`select id from favoritesRecipes where id='${recipID}' and user_id='${req.session.user_id}' `);

    if (check_if_exist2.length == 1) {
      await DButils.execQuery(`update favoritesRecipes set watched=1  where id='${recipID}'`);
    }
...
async function relevantData(recipe, req) {
  ...
  if (id === undefined) {
    ...
    let is_watched = await isUserWatched(recipe.data.id)
    let is_favourite = await isInFavourite(recipe.data.id)
    if (instructions.length > 0) {
      return {
        id: recipe.data.id,
        title: recipe.data.title,
